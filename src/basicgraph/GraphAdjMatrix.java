...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getInNeighbors(int v) {
		...
		for (int i = 0; i < getNumVertices(); i ++) {
			for (int j=0; j< adjMatrix[i][v]; j++) {
				inNeighbors.add(i);
				//System.out.println("\n J: " + j + " v:" + v +" adjMatrix[i][v]:" + adjMatrix[i][v]);
			}
		}
		return inNeighbors;
	}
	...
	public List<Integer> getDistance2(int v) {
		...
		List<Integer> oneHopVerts = getNeighbors(v);
		List<Integer> twoHopVerts = new ArrayList<>() ;

		for ( int w : oneHopVerts){
			List<Integer> dist2list = getNeighbors(w);
			for (int z : dist2list){
				twoHopVerts.add(z);
			}
		}
		...
		/*
		int[][] m2 = new int[getNumVertices()][getNumVertices()];
		for (int i=0 ; i < getNumVertices(); i ++){
			for (int j = 0 ; j < getNumVertices() ; j++){
				m2[i][j]+= adjMatrix[i][j] * adjMatrix[j][i];
			}
		}
		for (int j=0 ; j < getNumVertices(); j++){
			if (m2[v][j] > 0) {
				twoHopVerts.add(m2[v][j]);
			}
		}
